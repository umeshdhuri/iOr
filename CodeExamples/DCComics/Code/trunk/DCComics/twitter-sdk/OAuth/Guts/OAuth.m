...
- (void) synchronousAuthorizeTwitterTokenWithVerifier:(NSString *)oauth_verifier {
	...
	if ([request error]) {
		...
		for (NSString *component in responseBodyComponents) {
            ...
            [userDefault setValue:[subComponents objectAtIndex:1] forKey:[subComponents objectAtIndex:0]];
            
		}
		
         NSLog(@"responseBodyComponents === %@", [userDefault valueForKey:@"screen_name"]);
    ...
    NSString *urlValue = [NSString stringWithFormat:@"https://api.twitter.com/1/users/show.json?screen_name=%@&include_entities=true", [userDefault stringForKey:@"screen_name"]];
